...
export const StudentPanel: React.FC = () => {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<'overview' | 'exams' | 'results' | 'fees' | 'courses' | 'timetable' | 'syllabus' | 'studylog' | 'documents'>('overview');
  ...
  const fetchStudentData = async () => {
    ...
    try {
      ...
      if (student) {
        ...
        const [examsRes, resultsRes, feesRes, transactionsRes, coursesRes, timetableRes, syllabusRes, studyLogsRes, documentsRes, settingsRes] = await Promise.all([
          ...
          >
            ...
            <div className="bg-white p-8 rounded-[32px] border border-primary/10 shadow-sm flex flex-col md:flex-row items-center justify-between gap-8">
              ...
              <div className="flex gap-4">
                <div className="px-6 py-4 bg-emerald-50 rounded-2xl border border-emerald-100 text-center">
                  ...
                  <p className="text-xl font-black text-emerald-700">42 Days</p>
                </div>
                <div className="px-6 py-4 bg-rose-50 rounded-2xl border border-rose-100 text-center">
                  ...
                  <p className="text-xl font-black text-rose-700">3 Days</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-[32px] border border-primary/10 shadow-sm overflow-hidden">
               ...
               <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {/* Fetched attendance will go here */}
                    <tr>
        ...
        {activeTab === 'documents' && (
          ...
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              <div className="lg:col-span-1 border-2 border-dashed border-primary/20 rounded-[32px] p-8 text-center hover:border-primary/40 transition-all bg-primary/5">
                <input 
                  type="file" 
                  id="doc-upload" 
                  className="hidden" 
                ...
                <label htmlFor="doc-upload" className="cursor-pointer block space-y-4">
                  ...
                  <div>
                    <h4 className="font-black text-slate-800">Upload Document</h4>
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">PDF, JPG up to 10MB</p>
                  </div>
                  <button className="w-full py-4 bg-primary text-white rounded-2xl font-black shadow-lg shadow-primary/20 pointer-events-none">
                    Select File
                  </button>
                </label>
              </div>
                ...
                <div className="bg-white rounded-[32px] border border-primary/10 shadow-sm overflow-hidden">
                  <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-black text-slate-800">Documents Received from College</h3>
                    </div>
                  </div>
                  <div className="divide-y divide-slate-50">
                    {(documents || []).filter(d => d.category === 'Official').map((doc) => (
                      <div key={doc.id} className="px-8 py-5 flex items-center justify-between group hover:bg-slate-50 transition-all">
                        <div className="flex items-center gap-4">
                    ...
                    {documents.filter(d => d.category === 'Official').length === 0 && (
                      <div className="p-12 text-center text-slate-400 font-bold">No official documents received.</div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="px-8 py-6 border-b border-slate-50">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-black text-slate-800">My Uploaded Documents</h3>
                      </div>
                    </div>
                  </div>
                  <div className="divide-y divide-slate-50">
                    ...
                    {documents.filter(d => d.category !== 'Official').length === 0 && (
                      <div className="p-12 text-center text-slate-400 font-bold">No personal documents uploaded yet.</div>
                    )}
                  </div>
                </div>
              </div>
